...
void saveIdsToFlash()
{
  ...
  {
    savedNumberOfUserIds = savedNumberOfUserIds + i;
    String idKey = String(i);
    // save each id to it's assigned number
    preferences.putString(idKey.c_str(), userIdsList[i]);
  }
  // updated count of ids stored of flash
  preferences.putInt("savedNumberOfUserIds", savedNumberOfUserIds);
  preferences.end();
  Serial.println("IDS STORED TO FLASH...UPDATED COUNT");
}
